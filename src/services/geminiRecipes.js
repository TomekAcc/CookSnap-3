...
7. PHYSICAL SENSE CHECK (MANDATORY — do this before finalizing every step): every step must be physically possible and use ingredients in their correct form. A raw ingredient must be raw when it's meant to be cooked, and a cooked ingredient must never be described as "poured", "poured in", or added as if it were a liquid when it can't be. Concretely: you pour RAW whisked eggs into a pan (they are liquid); you never "pour" a cooked/solid egg or a solid ingredient. Re-read each step and check "can a person actually do this with the ingredient in the state it would be in at that point?" — if not, fix it.
8. Use a single measurement system (US or Metric) as specified — never mix or show both. When the recipe language is not English, translate the unit words too (e.g. "łyżka" not "tbsp").
9. Match the step count to the dish's complexity — 3 steps for simple dishes, 4 for standard, 5 for more involved ones. Never pad a simple dish to 5 steps.
10. Write clear, imperative instructions. Every step must be a complete sentence ending with a period. Every ingredient used in a step must appear in the ingredientsList.
11. Across the batch of 5, vary the dish formats and avoid repeating the same dishType. Don't use the same lead flavor word in more than one title.
12. Titles: 2-4 words, specific and recognizable. Use a real dish-type word (salad, soup, pasta, stir-fry, omelet, curry, etc.) — never a vague container word like "Plate", "Dish", "Meal", "Mix", or "Medley". In non-English languages, don't mechanically translate "Bowl" or "Skillet" as a head noun; use the language's own real dish-type vocabulary.

REQUIRED FIELDS PER RECIPE OBJECT:
- "id": number (1..5)
- "title": Dish Name
- "dishType": one of "pasta" | "noodles" | "bread" | "pancake" | "oatmeal" | "salad" | "omelet" | "eggs" | "coffee" | "chocolate" | "pudding" | "cake" | "sweet_bowl" | "soup" | "rice" | "chicken" | "meat" | "sandwich" | "other"
...
- "title": appetizing catchy dish name in ${language} (MANDATORY). 2-4 words max, one flavor/prep word plus 1-2 key ingredients plus dish type — see TITLE NAMING RULES above (the naming SHAPE applies in any language; only the words themselves change). Never list every ingredient in the title. Across this batch of ${recipeCount}, every title's lead descriptor word must be DIFFERENT — do not open two or more titles with the same flavor/prep word, even if several dishes share a quality like sweetness. ${isEnglish ? `The title's LAST word (its dish-type word) must be a real, specific, recognizable format that genuinely matches what's cooked — never a vague word like "Plate", "Dish", or "Meal", and never paired with a lead ingredient that makes no real dish` : `The title must genuinely name a real, recognizable dish — see the TITLE DISH-TYPE WORD block above's non-English guidance in full before writing this: DO NOT mechanically translate an English container word like "Bowl"/"Skillet" into ${language} and place it as the title's head noun (this is a confirmed real, repeated failure — a literal "Frying-Pan" translation as the title's subject is not how a real ${language} speaker names a dish). Real food-type words (soup, salad, curry, pasta, ...) translate fine as the trailing word; container metonyms do not — use ${language}'s own real dish-type vocabulary where one fits, or drop the trailing format word entirely and name the dish by its protein/technique/vegetables the way a native ${language} speaker actually would`} (see the TITLE DISH-TYPE WORD block above — this is the single most common quality failure, check it for every title before finalizing). CAPITALIZATION: ${isEnglish ? `standard English Title Case — capitalize each major word ("Creamy Garlic Chicken"), which is what this whole example set already shows.` : isGerman ? `German's own real rule — every noun is capitalized wherever it falls in the sentence (this is correct German grammar, not the English-Title-Case rule applied to a language that doesn't use it); all other words follow German's own normal capitalization (first word of a sentence/title, proper nouns).` : `the language's own real capitalization rule — typically only the first word and proper nouns are capitalized, never every major word like English does.`}
