...
6. Use a single measurement system (US or Metric) as specified — never mix or show both. When the recipe language is not English, translate the unit words too (e.g. "łyżka" not "tbsp").
7. Match the step count to the dish's complexity — 3 steps for simple dishes, 4 for standard, 5 for more involved ones. Never pad a simple dish to 5 steps.
8. Write clear, imperative instructions. Every step must be a complete sentence ending with a period. Every ingredient used in a step must appear in the ingredientsList.
9. Across the batch of 5, vary the dish formats and avoid repeating the same dishType. Don't use the same lead flavor word in more than one title.
10. Titles: 2-4 words, specific and recognizable. Use a real dish-type word (salad, soup, pasta, stir-fry, omelet, curry, etc.) — never a vague container word like "Plate", "Dish", "Meal", "Mix", or "Medley". In non-English languages, don't mechanically translate "Bowl" or "Skillet" as a head noun; use the language's own real dish-type vocabulary.
