...
- NATIVE GRAMMAR & FLUENCY (MANDATORY — THIS IS THE MOST COMMON FAILURE): When ${language} is not English, write everything the way a native ${language} speaker who cooks would naturally phrase it. This means:
  ...
  * ZERO ENGLISH IN NON-ENGLISH RECIPES (MANDATORY — THIS IS A CONFIRMED REAL FAILURE): When ${language} is not English, NO natural-language text — especially titles and ingredient names — may contain any English words, even common English food words. This is a confirmed real failure: Polish titles came out as "Zesty Lemon", "Lemon Yoghurt Poached Egg Bowl", and "Red ..." — literally untranslated English. That is completely unacceptable for a user reading in Polish; it looks broken and untrustworthy. Before finalizing any title or ingredient, check EVERY word: if it is an English word that has a native ${language} equivalent, you MUST use the ${language} word (e.g. "Lemon" → "Cytryna", "Yoghurt" → "Jogurt", "Egg" → "Jajko", "Poached" → "w koszulce", "Bowl" → use ${language}'s real dish-type vocabulary). The only words that stay English are the fixed JSON field keys, "category", and "dishType" tokens. If you catch yourself writing an English word in a title or ingredient for a non-English recipe, stop and translate it.
