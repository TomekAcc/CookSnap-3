...
5. ZONE 5 (LOWER DOOR SHELVES): "Łaciate" carton -> "Milk". Yellow/white plant-milk carton -> "Oat Milk". Dark glass bottles when oil type is clear -> "Olive Oil" / "Sesame Oil". Juice cartons/bottles -> identify the SPECIFIC fruit, not just "Juice": the carton's own color/label/imagery almost always shows this (orange carton with an orange -> "Orange Juice", a red/purple carton or label showing berries/apples -> "Apple Juice" / "Cranberry Juice" / whatever fruit is actually depicted). This matters — a recipe built around apple juice is a genuinely different dish than one built around orange juice, so a generic "Juice" is not specific enough to be useful. Only fall back to a plain "Juice" when the flavor genuinely cannot be determined from the packaging at all (fully plain/unlabeled bottle) — same STRICT VISIBILITY RULE as everything else: read the real flavor off the packaging, never guess one that isn't shown.
